...
$(document).ready(function () {
    ...
    // ANVA: we need different sorting methods for each type of data in the table
    var sortKeys = {
        alpha: function ($cell) { // this one sorts by alphabetic order
            var key = $cell.find('span.sort-key').text() + ' ';
            key += $.trim($cell.text()).toUpperCase();
            return key;
        },
        numeric: function ($cell) { // this one sorts numbers
        ...
        date: function ($cell) { // this one sorts datesﬂ
            //var key = Date.parse('1 ' + $cell.text());
            var key = $.trim($cell.text()).length;
            //console.log(key);
            return key;
        }
    };
